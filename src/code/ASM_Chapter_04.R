## Code from file Chap 4_v.13.docx
## Datetime: 2023-06-27 11:04:30
...
out4.4 <- jags(data = dataList, inits = inits, parameters.to.save = params, model.file = "model4.4.txt", n.iter = ni, n.burnin = nb, n.chains = nc, n.thin = nt, n.adapt = na, parallel = FALSE)


# Call JAGS in parallel (ART <1 min) and check convergence
out4.4 <- jags(data = dataList, inits = inits, parameters.to.save = params, model.file = "model4.4.txt", n.iter = ni, n.burnin = nb, n.chains = nc, n.thin = nt, n.adapt = na, parallel = TRUE)
...
which(out4.4$summary[,8] > 1.1) # check for non-convergence: none in this case
...
for (i in pars){
  ...
  plot(density(samps), type = 'l', lwd = 2, col = 'gray40', main = "", xlab = i, frame = FALSE)
}
...
print(cbind(truth = truth, lm = lm_est, JAGS = jags_est), 4)
...
# Call NIMBLE (ART 21 sec)
system.time(
  out4.5 <- nimbleMCMC(code = model4.5,
...
par(mfrow=c(1, 3)); coda::traceplot(out4.5$samples) # not shown
...
# Definition of NLL for a normal linear model
NLL <- function(param, y) {
  pop.mean <- param[1]      # First parameter is mean (mu)
  pop.sd <- param[2]        # Second is SD (sigma)
  -sum(dnorm(y, mean = pop.mean, sd = pop.sd, log = TRUE)) # Note minus sign
}
